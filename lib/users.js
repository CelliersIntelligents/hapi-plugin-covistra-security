...
module.exports = function(plugin, log, config) {
    ...
    function _getByUsername(username) {
        ...
        var coll = db.collection(COLNAME);
        return P.promisify(coll.findOne, coll)({$or: [{username: username}, {email: username}]}).then(function(data) {
            if(data) {
    ...
    function _create(data) {
        ...
        if(data.status === STATUS_VALUES.NEW) {
            ...
            data.status = data.status || STATUS_VALUES.PENDING_EMAIL_VERIFICATION;
        }
